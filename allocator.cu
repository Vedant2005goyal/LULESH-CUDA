...
template <typename T> inline T* allocate(int size) { return new T(size); }
...
template <typename T> T* Allocator<T>::free(T* v, int size) {
...
template <typename T> void Allocator<T>::clear() {
  {
