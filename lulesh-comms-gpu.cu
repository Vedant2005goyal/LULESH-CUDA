...
template <int type>
__global__ void SendPlane(Real_t* destAddr, Real_t* srcAddr, Index_t sendCount,
...
template <int type>
__global__ void AddPlane(Real_t* srcAddr, Real_t* destAddr, Index_t recvCount,
                         Index_t dx, Index_t dy, Index_t dz) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= recvCount)
    return;
...
template <int type>
__global__ void CopyPlane(Real_t* srcAddr, Real_t* destAddr, Index_t recvCount,
                          Index_t dx, Index_t dy, Index_t dz) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid >= recvCount)
    return;
...
template <int type>
__global__ void SendEdge(Real_t* destAddr, Real_t* srcAddr, Index_t sendCount,
                         Index_t dx, Index_t dy, Index_t dz) {
...
template <int type>
__global__ void AddEdge(Real_t* srcAddr, Real_t* destAddr, Index_t recvCount,
                        Index_t dx, Index_t dy, Index_t dz) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= recvCount)
    return;
...
template <int type>
__global__ void CopyEdge(Real_t* srcAddr, Real_t* destAddr, Index_t recvCount,
                         Index_t dx, Index_t dy, Index_t dz) {
  int i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i >= recvCount)
    return;
...
__global__ void AddCorner(Real_t* destAddr, Real_t src) { destAddr[0] += src; }

__global__ void CopyCorner(Real_t* destAddr, Real_t src) { destAddr[0] = src; }
...
void CommSendGpu(Domain& domain, int msgType, Index_t xferFields,
  ...
  if (planeMin | planeMax) {
    ...
    if (planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        ...
        SendPlane<0><<<(sendCount + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
        d_destAddr += sendCount;
      }
      d_destAddr -= xferFields * sendCount;
      cudaMemcpyAsync(destAddr, d_destAddr,
    ...
    if (planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendPlane<1><<<(sendCount + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
        d_destAddr += sendCount;
      }
      d_destAddr -= xferFields * sendCount;
  ...
  if (rowMin | rowMax) {
    ...
    if (rowMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendPlane<2><<<(sendCount + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
        d_destAddr += sendCount;
      }
      d_destAddr -= xferFields * sendCount;
    ...
    if (rowMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendPlane<3><<<(sendCount + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
        d_destAddr += sendCount;
      }
      d_destAddr -= xferFields * sendCount;
  ...
  if (colMin | colMax) {
    ...
    if (colMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendPlane<4><<<(sendCount + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
        d_destAddr += sendCount;
      }
      d_destAddr -= xferFields * sendCount;
    ...
    if (colMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendPlane<5><<<(sendCount + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), sendCount, dx, dy, dz);
        d_destAddr += sendCount;
      }
      d_destAddr -= xferFields * sendCount;
  ...
  if (!planeOnly) {
    if (rowMin && colMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<0><<<(dz + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dz, dx, dy, dz);
        d_destAddr += dz;
      }
      d_destAddr -= xferFields * dz;
      cudaMemcpyAsync(destAddr, d_destAddr, xferFields * dz * sizeof(Real_t),
                      cudaMemcpyDeviceToHost, stream);
      cudaStreamSynchronize(stream);
    ...
    if (rowMin && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<1><<<(dx + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dx, dx, dy, dz);
        d_destAddr += dx;
      }
      d_destAddr -= xferFields * dx;
      cudaMemcpyAsync(destAddr, d_destAddr, xferFields * dx * sizeof(Real_t),
                      cudaMemcpyDeviceToHost, stream);
      cudaStreamSynchronize(stream);
    ...
    if (colMin && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<2><<<(dy + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dy, dx, dy, dz);
        d_destAddr += dy;
      }
      d_destAddr -= xferFields * dy;
      cudaMemcpyAsync(destAddr, d_destAddr, xferFields * dy * sizeof(Real_t),
                      cudaMemcpyDeviceToHost, stream);
      cudaStreamSynchronize(stream);
    ...
    if (rowMax && colMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<3><<<(dz + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dz, dx, dy, dz);
        d_destAddr += dz;
      }
      d_destAddr -= xferFields * dz;
    ...
    if (rowMax && planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<4><<<(dx + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dx, dx, dy, dz);
        d_destAddr += dx;
      }
      d_destAddr -= xferFields * dx;
    ...
    if (colMax && planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<5><<<(dy + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dy, dx, dy, dz);
        d_destAddr += dy;
      }
      d_destAddr -= xferFields * dy;
    ...
    if (rowMax && colMin && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<6><<<(dz + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dz, dx, dy, dz);
        d_destAddr += dz;
      }
      d_destAddr -= xferFields * dz;
    ...
    if (rowMin && planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<7><<<(dx + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dx, dx, dy, dz);
        d_destAddr += dx;
      }
      d_destAddr -= xferFields * dx;
    ...
    if (colMin && planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<8><<<(dy + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dy, dx, dy, dz);
        d_destAddr += dy;
      }
      d_destAddr -= xferFields * dy;
    ...
    if (rowMin && colMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<9><<<(dz + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dz, dx, dy, dz);
        d_destAddr += dz;
      }
      d_destAddr -= xferFields * dz;
    ...
    if (rowMax && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<10><<<(dx + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dx, dx, dy, dz);
        d_destAddr += dx;
      }
      d_destAddr -= xferFields * dx;
    ...
    if (colMax && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        SendEdge<11><<<(dy + block - 1) / block, block, 0, stream>>>(
            d_destAddr, &(domain.*src)(0), dy, dx, dy, dz);
        d_destAddr += dy;
      }
      d_destAddr -= xferFields * dy;
    ...
    if (rowMin && colMin && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        cudaMemcpyAsync(&comBuf[fi], &(domain.*fieldData[fi])(0),
    ...
    if (rowMin && colMin && planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        cudaMemcpyAsync(&comBuf[fi], &(domain.*fieldData[fi])(idx),
                        sizeof(Real_t), cudaMemcpyDeviceToHost, stream);
      }
      cudaStreamSynchronize(stream);
      MPI_Isend(comBuf, xferFields, baseType, toRank, msgType, MPI_COMM_WORLD,
...
void CommSBNGpu(Domain& domain, int xferFields, Domain_member* fieldData,
  ...
  if (planeMin | planeMax) {
    ...
    if (planeMin) {
      ...
      cudaMemcpyAsync(d_srcAddr, srcAddr, xferFields * opCount * sizeof(Real_t),
                      cudaMemcpyHostToDevice, stream);
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        AddPlane<0><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
    }
    if (planeMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        AddPlane<1><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
  ...
  if (rowMin | rowMax) {
    ...
    if (rowMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        AddPlane<2><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
    ...
    if (rowMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        AddPlane<3><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
  ...
  if (colMin | colMax) {
    ...
    if (colMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        AddPlane<4><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
    ...
    if (colMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        AddPlane<5><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
  ...
  if (rowMin & colMin) {
    ...
    cudaMemcpyAsync(d_srcAddr, srcAddr, xferFields * dz * sizeof(Real_t),
                    cudaMemcpyHostToDevice, stream);
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<0><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  }
  ...
  if (rowMin & planeMin) {
    ...
    d_srcAddr =
        &domain.d_commDataRecv[pmsg * maxPlaneComm + emsg * maxEdgeComm];
    MPI_Wait(&domain.recvRequest[pmsg + emsg], &status);
    cudaMemcpyAsync(d_srcAddr, srcAddr, xferFields * dx * sizeof(Real_t),
                    cudaMemcpyHostToDevice, stream);
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<1><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  }
  ...
  if (colMin & planeMin) {
    ...
    d_srcAddr =
        &domain.d_commDataRecv[pmsg * maxPlaneComm + emsg * maxEdgeComm];
    MPI_Wait(&domain.recvRequest[pmsg + emsg], &status);
    cudaMemcpyAsync(d_srcAddr, srcAddr, xferFields * dy * sizeof(Real_t),
                    cudaMemcpyHostToDevice, stream);
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<2><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  }
  ...
  if (rowMax & colMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<3><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMax & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<4><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMax & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<5><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  ...
  if (rowMax & colMin) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<6><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMin & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<7><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMin & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<8><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  ...
  if (rowMin & colMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<9><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMax & planeMin) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<10><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMax & planeMin) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      AddEdge<11><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
...
void CommSyncPosVelGpu(Domain& domain, cudaStream_t* streams) {
  ...
  if (planeMin | planeMax) {
    ...
    if (planeMin && doRecv) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        CopyPlane<0><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
    ...
    if (planeMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        CopyPlane<1><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
  ...
  if (rowMin | rowMax) {
    ...
    if (rowMin && doRecv) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        CopyPlane<2><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
    ...
    if (rowMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        CopyPlane<3><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
  ...
  if (colMin | colMax) {
    ...
    if (colMin && doRecv) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        CopyPlane<4><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
    ...
    if (colMax) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        CopyPlane<5><<<(opCount + block - 1) / block, block, 0, stream>>>(
            d_srcAddr, &(domain.*dest)(0), opCount, dx, dy, dz);
        d_srcAddr += opCount;
      }
      ++pmsg;
  ...
  if (rowMin && colMin && doRecv) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<0><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMin && planeMin && doRecv) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<1><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMin && planeMin && doRecv) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<2><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  ...
  if (rowMax & colMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<3><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMax & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<4><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMax & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<5><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  ...
  if (rowMax & colMin) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<6><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMin & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<7><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMin & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<8><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  ...
  if (rowMin && colMax && doRecv) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<9><<<(dz + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dz, dx, dy, dz);
      d_srcAddr += dz;
    }
    ++emsg;
  ...
  if (rowMax && planeMin && doRecv) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<10><<<(dx + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dx, dx, dy, dz);
      d_srcAddr += dx;
    }
    ++emsg;
  ...
  if (colMax && planeMin && doRecv) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      CopyEdge<11><<<(dy + block - 1) / block, block, 0, stream>>>(
          d_srcAddr, &(domain.*dest)(0), dy, dx, dy, dz);
      d_srcAddr += dy;
    }
    ++emsg;
  ...
  if (rowMin & colMin & planeMax) {
    ...
    MPI_Wait(&domain.recvRequest[pmsg + emsg + cmsg], &status);
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      CopyCorner<<<1, 1, 0, stream>>>(&(domain.*fieldData[fi])(idx),
                                      comBuf[fi]);
    }
    ++cmsg;
  }
    ...
    if (planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        ...
        cudaMemcpyAsync(&(domain.*dest)(fieldOffset[fi]), srcAddr,
                        opCount * sizeof(Real_t), cudaMemcpyHostToDevice,
                        stream);
        srcAddr += opCount;
